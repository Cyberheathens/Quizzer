      ...
      {quizInfo && activeTab === 'poll' && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="px-4 pt-3">
          <div className="glass rounded-xl px-4 py-2.5 flex items-center justify-between">
            <span className="text-xs font-semibold text-flame">{quizInfo.title}</span>
            <span className="chip !py-1">Q {quizInfo.order_index + 1} / {quizInfo.total}</span>
          </div>
        </motion.div>
      )}
